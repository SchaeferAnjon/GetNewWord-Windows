...
2. 对每个单词提供：
   ...
   - contextMeaning：当前语境对应的那个义项（**不带词性前缀**，必须与 meaning 里该义项的写法**逐字一致**，用于高亮）。**先定这个义项，后面的译文和 translationKeyword 都必须围绕它**
   - contextSentence：红框所在的**完整原文句子**（从红框外的上下文里补全，不要截断）
   - 该句子的中文翻译（目标词在译文中的译法**必须采用 contextMeaning 的措辞或其核心词**，用于译文高亮）
   - translationKeyword：目标词在译文中对应的那几个字，**逐字摘抄自译文**（如译文"这包括在…"里对应 involve 的是"包括"）；译文中没有明确对应就给 ""。**一致性硬约束**：contextMeaning、译文措辞、translationKeyword 三者必须是同一个义项——若你想在译文里用"接受"，contextMeaning 就不能选"赞同（社会层面）"，反之亦然；不一致时改其中一方对齐
